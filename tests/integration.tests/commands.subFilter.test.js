...
describe('The Subscription Filter Command', function() {
    ...
    it('Delete filter at number X', function() {
        const chatID = '1234';

        payload.command.params = 'delete 2';

        let promises = messengers.map((messenger, i) => 
            command(payload, messenger)
        );

        return Promise.all(promises)
        .then(res => {
            // expect(res).to.all.have.property('text', 'Successfully deleted subscription filter: All drops');
        ...
        .then(res => {
            expect(res).to.have.lengthOf(2);
            expect(res).to.have.deep.members([
                { chatID, filters: [ {events: ['sale', 'bid'], txPriceGTE: '$325'} ], messenger: 'twitter' },
                { chatID, filters: [ {events: ['sale', 'bid'], txPriceGTE: '$325'} ], messenger: 'telegram' },
            ]);
        });
    });
});
